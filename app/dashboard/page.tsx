...
export default async function DashboardPage() {
  ...
  // Admins see all public courses; students see public courses + their own private ones
  // RLS handles this automatically via the "Courses viewable by creator or if public" policy
  const { data: courses } = await supabase
    .from("courses")
    .select("*, modules(*)")
    .order("created_at", { ascending: false });

  const systemCourses = courses?.filter((course) => course.is_public);
  const myCourses = courses?.filter(
    (course) => course.creator_id === profile?.id,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {myCourses && myCourses.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center p-12 border-2 rounded-2xl border-dashed border-gray-300 dark:border-gray-700">
      ...
      {systemCourses && systemCourses.length > 0 && (
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Explore all other courses in our syllabus
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
